    ...
    return new Vuex.Store({
        ...
        actions: {
            async nuxtServerInit(VuexContext, context) {
                const order1Response = await context.$axios.get(process.env.API + 'articles?filter[rubric.order]=1&sort=-posted_at&itemsPerPage=10&include=rubric')
                VuexContext.commit('setOrder1', order1Response.data.data.data)

                const randomArticlesResponse = await context.$axios.get(process.env.API + 'random-articles?itemsPerPage=9&include=rubric')
                VuexContext.commit('setRandomArticles', randomArticlesResponse.data.data)

                const order2Response = await context.$axios.get(process.env.API + 'articles?filter[rubric.order]=2&sort=-posted_at&itemsPerPage=4&include=rubric')
                VuexContext.commit('setOrder2', order2Response.data.data.data)

                const order3Response = await context.$axios.get(process.env.API + 'articles?filter[rubric.order]=3&sort=-posted_at&itemsPerPage=6&include=rubric,author')
                VuexContext.commit('setOrder3', order3Response.data.data.data)

                const order4Response = await context.$axios.get(process.env.API + 'articles?filter[rubric.order]=4&sort=-posted_at&itemsPerPage=4&include=rubric,author')
                VuexContext.commit('setOrder4', order4Response.data.data.data)

                const order5Response = await context.$axios.get(process.env.API + 'articles?filter[rubric.order]=5&sort=-posted_at&itemsPerPage=4&include=rubric')
